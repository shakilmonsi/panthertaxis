...
const testimonialsData = [
  {
    quote:
      "VehicleCheck Pro has transformed our daily operations. The digital forms save us hours each week, and the document reminders ensure we never miss renewals.",
    author: "Sarah Mitchell",
    title: "Fleet Manager, City Transport",
...
const TestimonialsSection = () => {
  return (
    <div className="flex w-full flex-col items-center justify-center gap-10 px-4 py-24 sm:px-12 md:px-16 lg:px-24">
      {/* Section Header */}
      <div className="flex w-full max-w-7xl flex-col items-center justify-start gap-4 text-center">
        <h1 className="text-3xl font-semibold text-neutral-800 sm:text-4xl md:text-5xl">
          Trusted by Professionals
        </h1>
        <p className="text-sm font-[600] text-[#555] sm:text-base">
          See what drivers and fleet managers are saying about VehicleCheck Pro.
        </p>
      </div>
      ...
      <div className="flex w-full max-w-7xl flex-col items-center justify-center gap-4 sm:flex-row sm:flex-wrap lg:flex-nowrap lg:justify-center">
        {testimonialsData.map((testimonial, index) => (
          <div
            key={index}
            className="flex h-64 w-full max-w-sm flex-shrink-0 flex-col items-start justify-between gap-4 rounded-xl bg-white p-6 shadow-[0px_4px_8px_0px_rgba(82,136,244,0.10)] md:w-96 lg:w-96"
          >
            {/* Quote and Stars */}
            <div className="flex flex-col items-start justify-start gap-2">
              <StarRating count={testimonial.stars} />
              <p className="self-stretch text-base leading-normal font-normal text-zinc-500">
                "{testimonial.quote}"
              </p>
            </div>
